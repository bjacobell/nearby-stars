...
def ET_power_law(verbose=False):
    ...
    telescope = ['VLA']
    GM_stars = 1e12
    GM_stars = np.array([1e12, 4e10, 1e12, 4e10])

    band = np.array([1e6, 0.125e6])
    band = np.array([1e6, 1e6, 0.125e6, 0.125e6])
    central_freq = np.array([1.4e9, 8.4e9])
    central_freq = np.array([1.42e9, 1.42e9, 1.42e9, 1.42e9])
    freq_range_norm = (band/central_freq)

    SEFD = calc_SEFD(calc_DishArea(25), 35, eff=0.45) / \
        np.sqrt(27*26.)  # Perley 2009
    m = 7.0
    nu = [122., 122., 15.3, 15.3] # 122 Hz in other-galaxy SR, 15 Hz in LSR
    t = [20*60, 20*60, 5*60, 5*60] # 20 min for shifted SR, 5 min for LSR
    Sens = np.array([calc_Sensitivity(m, nu[i], t[i], SEFD=SEFD) for i in range(len(nu))])

    dist_std = (2.5e6 * u.lyr.to('m'))  # Max distance approx
    dist_std = np.array([(2.5e6 * u.lyr.to('m')), 
                         (3.2e6 * u.lyr.to('m')),
                         (2.5e6 * u.lyr.to('m')), 
                         (3.2e6 * u.lyr.to('m'))])
    GM_EIRP = np.array([calc_EIRP_min(dist_std[i], Sens[i]) for i in range(len(Sens))])
    ...
    # 0.95deg images    #NOTE: in Enriquez 2017, we used this as the radius, but it is the diameter.
    GM_sky = 8*(0.95/2.)**2*np.pi
    ...
    if verbose:
        ...
        print('GM17: 1/rarity:', 1/GM_rarity)
        print('BeamSize (Gray & Mooley 2017):', end=' ')
        print('Sky Coverage (Gray & Mooley 2017):', GM_sky)
        print('CWTFM (Gray & Mooley 2017):',  zeta_AO * (GM_EIRP_tot) /
              (GM_rarity_tot))  # ,'or', zeta_AO*stats.hmean(GM_EIRP/GM_rarity)
    ...
    band = np.array([(1.75-1.2)*1e9 - 180e6, (3.0-1.75)*1e9,
                     (1.75-1.2)*1e9, (3.0-1.75)*1e9, (3.0-1.2)*1e9])
    ...
    m = 1
    nu = 1.0
    t = [276, 195, 276, 138, 552]
    Sens1 = np.array([calc_Sensitivity(
        m, nu, t[i], SEFD=SEFD[i], narrow=False) for i in range(len(SEFD))])
    ...
    iband = 20e6
    # Note: This value is calculated with self calculated SEFD values (which are not completely consistent with values expected from Harp 2016 values).
    Ph_speed = SEFD.mean()**2*nu/iband

    Ph_sky = Ph_stars * \
        np.array([calc_BeamSize(Dish_D[i], central_freq[i])
                  for i in range(len(Dish_D))])
    ...
    if verbose:
        ...
        print('BeamSize (Phoenix):', np.array(
            [calc_BeamSize(Dish_D[i], central_freq[i]) for i in range(len(Dish_D))]))
        print('Sky Coverage (Phoenix):', Ph_sky.sum())
        print('CWTFM (Phoenix):',  zeta_AO * (Ph_EIRP) / (Ph_rarity))
        print('rarity (Phoenix):', Ph_rarity_tot)
    ...
    # There are 73MHz which are RFI flagged, it is ignored here.
    band = np.array([8000.e6, 2040.e6, 337.e6, 268.e6])
    ...
    nu = 0.7
    t = 93.
    dist_std = np.array([(1.4e3*3.26156 * u.lyr.to('m')), (1.1e3*3.26156 * u.lyr.to(
        'm')), (300 * u.lyr.to('m')), (500 * u.lyr.to('m'))])  # Turnbull 2003 for HabCat
    Sens = np.array(
        [calc_Sensitivity(m, nu, t, SEFD=SEF, narrow=False) for SEF in SEFDs])
    ATA_EIRP = np.array([calc_EIRP_min(dist_std[i], Sens[i])
    ...
    iband = 800e6
    # 0.85 ==> Siemion priv. comm. (due to 2 bit data format)
    Siemion_speed = (SEFD/0.85)**2*nu/iband
    ...
    if verbose:
        ...
        print('rarity (Siemion2013):', Siemion_rarity)
        print('BeamSize (Siemion2013):', calc_BeamSize(100, 1.5e9))
        print('Sky Coverage (Siemion2013):', Siemion_sky)
        print('CWTFM (Siemion2013):',  zeta_AO *
              (Siemion_EIRP) / (Siemion_rarity))
    ...
    nu = [4883., 76.]
    t = 3000.
    Sens = np.array([calc_Sensitivity(m, nu[0], t, SEFD=SEFD, npol=1.),
                     calc_Sensitivity(m, nu[1], t, SEFD=SEFD, npol=1.)])
    ...
    if verbose:
        ...
        print('CWTFM (Valdes 1986):',  zeta_AO *
              (Valdes_EIRP_tot) / (Valdes_rarity_tot))
    ...
    if verbose:
        ...
        print('CWTFM (Tarter1980):',  zeta_AO *
              (Tarter_EIRP) / (Tarter_rarity))
    ...
    SEFD = np.array([calc_SEFD(calc_DishArea(300*0.3048), 110, eff=0.75), calc_SEFD(
        calc_DishArea(140*0.3048), 48, eff=0.75)])  # **NOTE** the 0.75 for the 140' is not real
    m = 3.0
    nu = [490., 7.2e3]
    t = [4*60., 5*60.]
    Sens = np.array([calc_Sensitivity(m, nu[0], t[0], SEFD=SEFD[0]),
                     calc_Sensitivity(m, nu[1], t[1], SEFD=SEFD[1])])
    ...
    # 300 ft: Two 192-channel receivers (at 130 km/s with 4.74kHz=1km/s at this freq.)
    iband = np.array([0.6e6, 2.5e6])
    Verschuur_speed = SEFD.min()**2*nu[0]/iband[0]

    # The two comes from the off beam.
    Verschuur_sky = (Verschuur_stars * np.array(
        [calc_BeamSize(300*0.3048, 1.42e9), calc_BeamSize(140*0.3048, 1.42e9)])).sum()*2
    ...
    if verbose:
        ...
        print('BeamSize (Verschuur1973):', np.array(
            [calc_BeamSize(300*0.3048, 1.42e9), calc_BeamSize(140*0.3048, 1.42e9)]))
        print('Sky Coverage (Verschuur1973):', Verschuur_sky)
        print('CWTFM (Verschuur1973):',  zeta_AO *
              (Verschuur_EIRP_tot) / (Verschuur_rarity_tot))
    ...
    # eff=0.5 ==>  We were unable to find a value in the literature. We assume a similar value to the antenna of the same dimensions from Valdes & Freitas (1986).
    SEFD = calc_SEFD(calc_DishArea(26), 85, eff=0.5)
    m = 30
    nu = 0.05
    t = 20
    Sens = calc_Sensitivity(m, nu, t, SEFD=SEFD, narrow=False)

    # Max distance: # Horowitz & Sagan (1993) suggested values for the number of stars given a distance, based on the power of an isotropic beacon.
    dist_std = (700*3.26156 * u.lyr.to('m'))
    ...
    if verbose:
        ...
        print('CWTFM (Horowitz):',  zeta_AO *
              (Horowitz_EIRP) / (Horowitz_rarity))
    ...
    Price_band = np.array(
        [(1.2-1.025+1.925-1.34)*1e9, (2.72-1.82)*1e9, (3.444-2.574)*1e9])
    ...
    Price_BL_SEFD = np.array([calc_SEFD(calc_DishArea(Dish_D[0]), 20, eff=0.72),
                              calc_SEFD(calc_DishArea(
                                  Dish_D[1]), 20, eff=0.72),
                              calc_SEFD(calc_DishArea(Dish_D[2]), 35, eff=0.7),
                              ])
    ...
    nu = 3.
    t = 300.
    Price_Sens = np.array([calc_Sensitivity(
        m, nu, t, SEFD=Price_BL_SEFD[i]) for i in range(len(Price_BL_SEFD))])

    dist_std = (50*3.26156 * u.lyr.to('m'))
    Price_BL_EIRP = np.array([calc_EIRP_min(dist_std, Sen)
                              for Sen in Price_Sens])
    ...
    Price_BL_sky = Price_BL_stars * \
        np.array([calc_BeamSize(Dish_D[i], Price_central_freq[i])
                  for i in range(len(Dish_D))])
    ...
    if verbose:
        ...
        print('rarity (Price_BL):', Price_BL_rarity)
        print('BeamSize (Price_BL):', np.array(
            [calc_BeamSize(Dish_D[i], Price_central_freq[i]) for i in range(len(Dish_D))]))
        print('Sky Coverage (Price_BL):', Price_BL_sky.sum())
        print('CWTFM (Price_BL):',  zeta_AO *
              (Price_BL_EIRP) / (Price_BL_rarity))
        print('CWTFM (Price_BL_tot):',  zeta_AO *
              (Price_EIRP_tot) / (Price_rarity_tot))
    ...
    tremblay_transmitter_rate_2 = np.log10(1.407e-7)
    tremblay_EIRP_min_2 = np.log10(1.942e17*1000)

    # ---------------------------------------------------------------------------------
    # Garrett and Siemion et al 2022
    # Multiple, some updates to other studies

    garrett_EIRP_min_interacting = 7.5e23*3.3
    garrett_log_TR_interacting = -10.9

    garrett_EIRP_min_group = 5.4e24*3.3
    garrett_log_TR_group = -11.2

    garrett_EIRP_min_AGN = 2.7e26*3.3
    garrett_log_TR_AGN = -10.6

    garrett_EIRP_min_galaxy = 9.8e23
    garrett_log_TR_galaxy = -10.6

    garrett_EIRP_min_lens = 7.9e29
    garrett_log_TR_lens = -10.6
    ...
    #solarpower, = plt.plot([16, 16], [-15, -8.6], '--',
    #                       lw=2, color='k', alpha=1)
    #solarpower1, = plt.plot([16, 16], [-6.3, 0], '--',
    #                        lw=2, color='k', alpha=1)
    plt.axvline(16, lw=2, ls='--', color='k', alpha=1)
    plt.text(16, -8, r'Kardashev Type I', {'va': 'center',
                                          'ha': 'right'}, rotation=90, fontsize=18)

    #arecibo, = plt.plot([13, 13], [-15, -9], lw=2,
    #                    color='k', alpha=1)
    #arecibo, = plt.plot([13, 13], [-6.1, 0], lw=2, color='k', alpha=1)
    plt.axvline(13, lw=2, color='k', alpha=1)
    plt.text(13, -8, r'Arecibo', {'va': 'center',
                                           'ha': 'right'}, rotation=90, fontsize=18)

    #kardashev_ii_1, = plt.plot([26, 26], [-15, -8.7], '--',
    #                           lw=2, color='k', alpha=1)
    #kardashev_ii_2, = plt.plot([26, 26], [-6.3, 0], '--',
    #                           lw=2, color='k', alpha=1)
    plt.axvline(26, lw=2, ls='--', color='k', alpha=1)
    plt.text(26, -8, r'Kardashev Type II', {'va': 'center',
                                           'ha': 'right'}, rotation=90, fontsize=18)
    ...
    price1, = plt.plot([np.log10(Price_BL_EIRP[2])], [np.log10(
        1./Price_BL_rarity[2])], 'd', color=SBand, markersize=markersize)
    price2, = plt.plot([np.log10(Price_EIRP_tot)], [np.log10(1./Price_rarity_tot)], 'd',
                       color=LBand, markersize=markersize, markeredgewidth=2, markeredgecolor=SBand)

    # -------
    enriquez, = plt.plot([np.log10(BL_EIRP)], [np.log10(1./BL_rarity)], 'd', color=LBand,
                         markeredgecolor=LBand, markersize=markersize, markeredgewidth=2)

    # Gray and Mooley did a lot at L-Band and 1 at X-Band
    graymooley, = plt.plot(np.log10(GM_EIRP[0]), np.log10(1./GM_rarity[0]), 'o', color=LBand, 
        #markeredgecolor=XBand, 
        #markeredgewidth=2, 
    ...
    harpall1, = plt.plot(np.log10(ATA_EIRP[0]), np.log10(1./ATA_rarity_tot), marker='d', color=CBand,
                         markersize=markersize, markeredgecolor=XBand, markeredgewidth=2, linestyle='None')
    harpall2, = plt.plot(np.log10(ATA_EIRP[0]), np.log10(1./ATA_rarity_tot), marker='d', color=LBand,
                         markersize=markersize-12, markeredgecolor=SBand, markeredgewidth=2, linestyle='None')
    ...
    siemion1, = plt.plot([np.log10(Siemion_EIRP)], [np.log10(
        1./Siemion_rarity)], 'd', color=LBand, markersize=markersize, linestyle='None')
    ...
    phoenixall, = plt.plot([np.log10(Ph_EIRP_tot)], [np.log10(
        1./Ph_rarity_tot)], marker='d', color=SBand, lw=0, markersize=markersize)

    horowitz_sagan, = plt.plot(np.log10(Horowitz_EIRP), np.log10(
        1./Horowitz_rarity), marker='s', color=SBand, markersize=markersize, linestyle='None')
    ...
    mwa, = plt.plot(tremblay_EIRP_min_2, tremblay_transmitter_rate_2, marker='d', color='tab:purple',
                    markersize=markersize, linestyle='None')  # , linestyle='None')

    garrett_siemion, = plt.plot(np.log10(garrett_EIRP_min_interacting),
                                garrett_log_TR_interacting, marker='o', color=LBand, markersize=markersize)
    garrett_siemion_2 = plt.plot(np.log10(
        garrett_EIRP_min_group), garrett_log_TR_group, marker='o', color=LBand, markersize=markersize)
    garrett_siemion_3 = plt.plot(np.log10(
        garrett_EIRP_min_AGN), garrett_log_TR_AGN, marker='o', color=LBand, markersize=markersize)
    #garrett_siemion_4 = plt.plot(np.log10(garrett_EIRP_min_lens), garrett_log_TR_lens, marker='P', color=LBand, markersize=markersize)
    #garrett_siemion_5 = plt.plot(np.log10(garrett_EIRP_min_galaxy), garrett_log_TR_galaxy, marker='P', color=LBand, markersize=markersize)

    uno_L, = plt.plot(np.log10(7.72e26*3.3), -14.5, marker='o',
                      color=LBand, markersize=markersize, lw=0)
    uno_S, = plt.plot(np.log10(7.89e26*3.3), -13.77, marker='o',
                      color=SBand, markersize=markersize, lw=0)
    uno_parkes_s, = plt.plot(np.log10(2.42e27*3.3), -12.85,
                             marker='o', color=SBand, markersize=markersize, lw=0)
    
    grex, = plt.plot(np.log10(8.5e24), -10.15, marker='s', color=LBand, markersize=markersize, lw=0)
    plt.plot(np.log10(1.0973452814254346e+24), -10.15, marker='s', color=LBand, alpha=0.25, markersize=markersize, lw=0)
    plt.plot(np.log10(2.8742604180588214e+22), -10.15, marker='s', color=LBand, alpha=0.25, markersize=markersize, lw=0)
    plt.plot(np.log10(3.710654243934994e+21), -10.15, marker='s', color=LBand, alpha=0.25, markersize=markersize, lw=0)

    legend_handles = {'p1': price1, 'p2': price2, 'e': enriquez, 'gm': graymooley, 'h_a1': harpall1,
                      's1': siemion1, 'pha': phoenixall, 'hs': horowitz_sagan, 'm': mwa, 'g': garrett_siemion, 'uno_L': uno_L, 'uno_S': uno_S, 'uno_parkes_s': uno_parkes_s, 
                      'grex': grex
                      }
...
def compare_SETI_limits(EIRP, rarity, shape='o', color='k', project='This Project', y_label_units=True, save_as=None):
    ...
    plt.plot([np.log10(EIRP)], [np.log10(1./rarity)], marker=shape,
             color=color, markersize=markersize, label=project)
    ET_power_law()
    ...
    image_filename = image_filename + \
        datetime.now().strftime("_%m-%d-%y_%H:%M:%S") + '.png'
