...
def sband_compare(max_distance, snr, save=False):
    # ---------------------------
    ...
    dish_diam = 100
    dish_Tsys = 22  # Telescope Tsys [Kelvin], GBT observers guide
    dish_app_eff = 0.68  # Telescope Apperture Efficiency
    ...
    if save:
        compare_SETI_limits(EIRP, survey_rarity, shape=shape, color=color,
                            project=project, y_label_units=y_label_units, save_as='SBand_seti_compare')

    sband_dict = {'EIRP': EIRP, 'EIRP_drift': EIRP_drift, 'rarity': survey_rarity, 'shape': shape,
