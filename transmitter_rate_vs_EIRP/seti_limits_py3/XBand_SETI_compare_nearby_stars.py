...
def xband_compare(max_distance, snr, save=False):
    # ---------------------------
    # Edit values below  vvvvvv
    # ---------------------------
    # Your new values ( e.g. Enriquez 2017  x 100)

    project = 'This Study: 4-Band'  # Project name
    telescope = 'GBT'     # Telescope name
    N_stars = 1180   # Estimated number of stars
    # band = 4200e6      # Total bandwidth [Hz]
    band = 4200e6 + 940e6 + 660e6 + 3800e6
    # central_freq = 9.9e9  # Central bandwidth [Hz]
    central_freq = 6.15e9
 
    # Telescope diameter meters (single dish in current version)
    dish_diam = 100
    dish_Tsys = 27  # Telescope Tsys [Kelvin], GBT observers guide
    dish_app_eff = 0.63  # Telescope Apperture Efficiency
    ...
    survey_sky = 1180 * calc_BeamSize(dish_diam, central_freq)
    ...
    if save:
        compare_SETI_limits(EIRP, survey_rarity, shape=shape, color=color,
                            project=project, y_label_units=y_label_units, save_as='XBand_seti_compare')

    xband_dict = {'EIRP': EIRP, 'EIRP_drift': EIRP_drift, 'rarity': survey_rarity, 'shape': shape,
