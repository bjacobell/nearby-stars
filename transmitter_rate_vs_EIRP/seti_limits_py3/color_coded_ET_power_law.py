...
def ET_power_law(verbose=False):
    ...
    nu = [122., 15.3]
    t = [20*60, 5*60]
    Sens = np.array([calc_Sensitivity(m, nu[0], t[0], SEFD=SEFD),
                     calc_Sensitivity(m, nu[1], t[1], SEFD=SEFD)])
    ...
    price1, = plt.plot([np.log10(Price_BL_EIRP[2])], [np.log10(
        1./Price_BL_rarity[2])], 'h', color=SBand, markersize=markersize)
    price2, = plt.plot([np.log10(Price_EIRP_tot)], [np.log10(1./Price_rarity_tot)], 'h',
    ...
    enriquez, = plt.plot([np.log10(BL_EIRP)], [np.log10(1./BL_rarity)], 'D', color=LBand,
                         markeredgecolor=LBand, markersize=markersize-8, markeredgewidth=2)

    # Gray and Mooley did a lot at L-Band and 1 at X-Band
    graymooley, = plt.plot(np.log10(GM_EIRP), np.log10(
        1./GM_rarity), 'o', color=LBand, markeredgecolor=XBand, markeredgewidth=2, markersize=markersize)
    ...
    harpall1, = plt.plot(np.log10(ATA_EIRP[0]), np.log10(1./ATA_rarity_tot), marker='^', color=CBand,
                         markersize=markersize, markeredgecolor=SBand, markeredgewidth=2, linestyle='None')
    ...
    siemion1, = plt.plot([np.log10(Siemion_EIRP)], [np.log10(
        1./Siemion_rarity)], '>', color=LBand, markersize=markersize, linestyle='None')
    ...
    phoenixall, = plt.plot([np.log10(Ph_EIRP_tot)], [np.log10(
        1./Ph_rarity_tot)], marker='<', color=SBand, markersize=markersize)

    horowitz_sagan, = plt.plot(np.log10(Horowitz_EIRP), np.log10(
        1./Horowitz_rarity), marker='s', color=SBand, markeredgecolor='w', markersize=markersize, linestyle='None')
    ...
    mwa, = plt.plot(tremblay_EIRP_min_2, tremblay_transmitter_rate_2, marker='X', color='tab:purple',
                    markersize=markersize, markeredgecolor='w', linestyle='None')  # , linestyle='None')

    garrett_siemion, = plt.plot(np.log10(garrett_EIRP_min_interacting),
                                garrett_log_TR_interacting, marker='P', color=LBand, markersize=markersize)
    garrett_siemion_2 = plt.plot(np.log10(
        garrett_EIRP_min_group), garrett_log_TR_group, marker='P', color=LBand, markersize=markersize)
    garrett_siemion_3 = plt.plot(np.log10(
        garrett_EIRP_min_AGN), garrett_log_TR_AGN, marker='P', color=LBand, markersize=markersize)
    ...
    uno_L, = plt.plot(np.log10(7.72e26*3.3), -14.5, marker='d',
                      color=LBand, markersize=markersize, lw=0)
    uno_S, = plt.plot(np.log10(7.89e26*3.3), -13.77, marker='d',
                      color=SBand, markersize=markersize, lw=0)
    uno_parkes_s, = plt.plot(np.log10(2.42e27*3.3), -12.85,
                             marker='v', color=SBand, markersize=markersize, lw=0)
    
    #grex, = plt.plot(np.log10(4.4e22), -10.15, marker='s', color=LBand, markersize=markersize, lw=0)
